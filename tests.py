...
class TestGeoCords(unittest.TestCase):
    def setUp(self):
        ...
        self.env = {
            'ip2w.config': {'IPINFO_TOKEN': self.api_token}
        }
        self.ip = '12.45.76.90'
    ...
    def test_empty_token(self):
        ...
        geo_resp2 = get_geo_cords({'ip2w.config': {}}, self.ip)
        self.assertTupleEqual(geo_resp2, (None, None))

    def test_success_response(self):
        lat, long = get_geo_cords(self.env, self.ip)
        float(lat)
        float(long)
...
class TestGetWeather(unittest.TestCase):
    def setUp(self):
        ...
        self.env = {
            'ip2w.config': {'OPENWEATHERMAP_TOKEN': self.api_token}
        }
    ...
    def test_empty_token(self):
        ...
        weather_resp2 = get_weather(
            {'ip2w.config': {'OPENWEATHERMAP_TOKEN': ''}},
            *self.cords
        )
    ...
    def test_bad_token(self):
        weather_resp = get_weather(
            {'ip2w.config': {'OPENWEATHERMAP_TOKEN': 'bad_token'}},
            *self.cords
        )
        self.assertIsNone(weather_resp)

    def test_success_response(self):
        weather_resp = get_weather(self.env, *self.cords)
